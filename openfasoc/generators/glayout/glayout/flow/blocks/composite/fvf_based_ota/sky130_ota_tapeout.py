...
def sky130_ota_add_pads(ota_in: Component, flatten=False) -> Component:
    ...
    if small_pad:
        ...
        pad = import_gds(
            "../../../../../tapeout/tapeout_and_RL/pads/Manhattan120umPad.gds"
        )
    ...
    ota_wpads << L_route(
        pdk,
        ota_wpads.ports["PLUS_top_met_E"],
        pad_array_ref.ports["row1_col3_pad_S"],
        hwidth=3,
        vwidth=3,
    )
    ota_wpads << L_route(
        pdk,
        ota_wpads.ports["MINUS_top_met_W"],
        pad_array_ref.ports["row1_col0_pad_S"],
        hwidth=3,
        vwidth=3,
    )
    ota_wpads << L_route(
        pdk,
        pad_array_ref.ports["row1_col1_pad_E"],
        ota_wpads.ports["VCC_top_met_N"],
        vwidth=4,
        hwidth=4,
        vglayer="met3",
        hglayer="met4",
    ...
    ota_wpads << L_route(
        pdk,
        ota_wpads.ports["VSS_top_met_E"],
        pad_array_ref.ports["row0_col3_pad_N"],
        hwidth=4,
        vwidth=4,
    )
    ota_wpads << L_route(
        pdk,
        ota_wpads.ports["IBIAS2_top_met_E"],
        pad_array_ref.ports["row0_col2_pad_N"],
        hwidth=3,
        vwidth=3,
    )
    ota_wpads << L_route(
        pdk,
        ota_wpads.ports["IBIAS1_top_met_W"],
        pad_array_ref.ports["row0_col1_pad_N"],
        hwidth=3,
        vwidth=3,
    )
    ota_wpads << L_route(
        pdk,
        ota_wpads.ports["DIFFOUT_top_met_W"],
        pad_array_ref.ports["row0_col0_pad_N"],
        hwidth=4,
        vwidth=4,
    )
...
# Port -> LVS-label mapping for the super class-AB OTA. Layers are derived from each
# port's own metal (the pin lands on the same layer the port is on, as LVS requires),
# reproducing the original hardcoded met2/met3 choices without hardcoding them.
_OTA_LABELS = [
    LabelSpec("AVSS", "VSS_top_met_N"),
    LabelSpec("NBC_10U", "IBIAS1_top_met_N"),
    LabelSpec("NB_10U", "IBIAS2_top_met_N"),
    LabelSpec("AVDD", "VCC_top_met_N"),
    LabelSpec("VOUT", "DIFFOUT_top_met_N"),
    LabelSpec("INP", "PLUS_top_met_N"),
    LabelSpec("INM", "MINUS_top_met_N"),
]


def sky130_add_ota_labels(ota_in: Component, pdk: MappedPDK = None) -> Component:
    """Add LVS pin/label shapes to the OTA. ``pdk`` defaults to sky130 so existing
    call sites (``sky130_add_ota_labels(ota_comp)``) are unchanged."""
    if pdk is None:
        from glayout.flow.pdk.sky130_mapped import sky130_mapped_pdk

        pdk = sky130_mapped_pdk
    return add_pin_labels(ota_in, pdk, _OTA_LABELS)
...
def sky130_add_ota_lvt_layer(ota_in: Component) -> Component:
    ...
    dimensions = (
        evaluate_bbox(ota_in)[0],
        (
            ota_in.ports["VCC_top_met_N"].center[1]
            - ota_in.ports["res_1_N_tie_S_top_met_S"].center[1]
        ),
    )

    lvt_rectangle = rectangle(layer=lvt_layer, size=(dimensions[0], dimensions[1]))
    lvt_rectangle_ref = prec_ref_center(lvt_rectangle)
    lvt_rectangle_ref.movey(
        ota_in.ports["res_1_N_tie_S_top_met_S"].center[1] + dimensions[1] / 2
    )
...
def ota_parameters_serializer(
    ...
    return np.array(
        [
            ...
            fvf_shunt_params[0],
            fvf_shunt_params[1],
            local_current_bias_params[0],
            local_current_bias_params[1],
            diff_pair_load_params[0],
            diff_pair_load_params[1],
            ratio,
            current_mirror_params[0],
            current_mirror_params[1],
            resistor_params[0],
            resistor_params[1],
            resistor_params[2],
            resistor_params[3],
            global_current_bias_params[0],
            global_current_bias_params[1],
            global_current_bias_params[2],
        ],
        dtype=np.float64,
    )
...
def ota_results_de_serializer(results: Optional[np.array] = None) -> dict:
    results_length_const = 11
    if results is None:
        results = results_length_const * [-987.654321]
    if not len(results) == results_length_const:
        raise ValueError(
            "results should be a length " + str(results_length_const) + " array"
        )
    results_dict = dict()
...
def get_small_parameter_list(test_mode=False) -> np.array:
    ...
    short_list = np.empty(
        shape=(short_list_len, len(ota_parameters_serializer())), dtype=np.float64
    )
    index = 0
...
def get_sim_results(
    acpath: Union[str, Path],
    dcpath: Union[str, Path],
    noisepath: Union[str, Path],
    slewpath: Union[str, Path],
):
...
def process_netlist_subckt(
    netlist: Union[str, Path],
    sim_model: Literal["normal model", "cryo model"],
    cload: float = 80.0,
    ...
    with open(netlist, "r") as spice_net:
        ...
        for i, line in enumerate(subckt_lines):
            ...
            if (
                (i + 1) < len(subckt_lines)
                and len(line)
                and len(subckt_lines[i + 1])
                and (subckt_lines[i + 1][0] == "+" or line[-1] == "+")
            ):
            ...
            if "cryo" in sim_model and len(line) > 1:
                subckt_lines[i] = subckt_lines[i].replace(
                    "sky130_fd_pr__nfet_01v8_lvt", "nshortlvth"
                )
                subckt_lines[i] = subckt_lines[i].replace(
                    "sky130_fd_pr__pfet_01v8_lvt", "pshort"
                )
                subckt_lines[i] = subckt_lines[i].replace(
                    "sky130_fd_pr__nfet_01v8", "nshort"
                )
                if (
                    ("nshort" in subckt_lines[i])
                    or ("pshort" in subckt_lines[i])
                    or ("nshortlvth" in subckt_lines[i])
                ):
            ...
            if ("floating" in line) or (
                noparasitics and len(line) and (line[0] == "c" or line[0] == "r")
            ):
...
def __run_single_brtfrc(
    ...
    cload: float = 80.0,
    noparasitics: bool = False,
    output_dir: Optional[Union[int, str, Path]] = None,
    hardfail=False,
):
    ...
    try:
        ota_v = sky130_add_ota_labels(
            sky130_add_ota_lvt_layer(super_class_AB_OTA(sky130pdk, **params))
        )
        ...
        with TemporaryDirectory() as tmpdirname:
            ...
            with open(
                str(_TAPEOUT_AND_RL_DIR_PATH_) + "/extract.bash.template", "r"
            ) as extraction_script:
                extractbash_template = extraction_script.read()
                extractbash_template = extractbash_template.replace(
                    "@@PDK_ROOT", PDK_ROOT
                ).replace("@@@PAROPT", "noparasitics" if noparasitics else "na")
            ...
            copyfile(
                str(_TAPEOUT_AND_RL_DIR_PATH_) + "/ota_perf_eval.sp",
                str(tmpdirname) + "/ota_perf_eval.sp",
            )
            copytree(
                str(_TAPEOUT_AND_RL_DIR_PATH_) + "/sky130A",
                str(tmpdirname) + "/sky130A",
            )
            # extract layouti
            Popen(
                ["bash", "extract.bash", tmp_gds_path, ota_v.name], cwd=tmpdirname
            ).wait()
            print(
                "Running simulation at temperature: " + str(temperature_info[0]) + "C"
            )
            process_spice_testbench(
                str(tmpdirname) + "/ota_perf_eval.sp", temperature_info=temperature_info
            )
            process_netlist_subckt(
                str(tmpdirname) + "/ota" + str(index) + "_pex.spice",
                temperature_info[1],
                cload=cload,
                noparasitics=noparasitics,
            )
            rename(
                str(tmpdirname) + "/ota" + str(index) + "_pex.spice",
                str(tmpdirname) + "/ota_pex.spice",
            )
        ...
        with open("get_training_data_ERRORS.log", "a") as errlog:
            errlog.write(
                "\nota run "
                + str(index)
                + " with the following params failed: \n"
                + str(params)
            )
    return results


def brute_force_full_layout_and_PEXsim(
    sky130pdk: MappedPDK,
    parameter_list: np.array,
    temperature_info: tuple[int, str] = (25, "normal model"),
    cload: float = 80.0,
    noparasitics: bool = False,
    saverawsims: bool = False,
) -> np.array:
    ...
    with Pool(128) as cores:
        if saverawsims:
            results = np.array(
                cores.starmap(
                    safe_single_build_and_simulation,
                    zip(
                        parameter_list,
                        repeat(temperature_info[0]),
                        count(0),
                        repeat(cload),
                        repeat(noparasitics),
                        repeat(False),
...
def get_training_data(
    ...
    cload: float = 80.0,
    noparasitics: bool = False,
    parameter_array: Optional[np.array] = None,
    saverawsims=False,
) -> None:
    ...
    results = brute_force_full_layout_and_PEXsim(
        pdk,
        params,
        temperature_info,
        cload=cload,
        noparasitics=noparasitics,
        saverawsims=saverawsims,
    )
...
def single_build_and_simulation(
    parameters: np.array,
    temp: int = 25,
    ...
    if temperature_info[0] > -20:
        ...
        raise ValueError(
            "simulation temperature should be exactly -269C for cryo sim. Below -20C there are no good models for simulation"
        )
    else:
    ...
    global pdk
    pdk = sky130_mapped_pdk
    results = __run_single_brtfrc(
        index,
        parameters,
        temperature_info=temperature_info,
        save_gds_dir=save_gds_dir,
    ...
    if (
        return_dict
    ):  # default behavoir will return a dictionary and filter phase margin below 45
...
class safe_single_build_and_simulation_helperclass:
    ...
    def execute(self):
        self.results = single_build_and_simulation(
            *self.passed_args, **self.passed_kwargs
        )
...
def safe_single_build_and_simulation(*args, **kwargs) -> dict:
    ...
    try:
        ...
        with open("get_training_data_ERRORS.log", "a") as errlog:
            errlog.write(
                "\nota run "
                + str(get_parameter_value("index", *args, **kwargs))
                + " with the following params failed: \n"
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="sky130 ota tapeout sample, RL generation, and statistics utility."
    )

    subparsers = parser.add_subparsers(title="mode", required=True, dest="mode")

    # Subparser for gen_ota mode
    gen_ota_parser = subparsers.add_parser(
        "gen_ota",
        help="Run the gen_ota function. optional parameters for transistors are width,length,current multiplication ratio",
    )
    gen_ota_parser.add_argument(
        "--input_pair_params",
        nargs=2,
        type=float,
        default=[4, 2],
        help="half_diffpair_params (default: 4 2)",
    )
    gen_ota_parser.add_argument(
        "--fvf_shunt_params",
        nargs=2,
        type=float,
        default=[2.75, 1],
        help="fvf_shunt_params (default: 2.75 1)",
    )
    gen_ota_parser.add_argument(
        ...
        type=float,
        default=[3.76, 3],
        help="local_current_bias_params (default: 3.76 3)",
    )
    gen_ota_parser.add_argument(
        ...
        type=float,
        default=[9, 1],
        help="diff_pair_load_params (default: 3.76 3)",
    )
    gen_ota_parser.add_argument(
        "--ratio", type=int, default=1, help="ratio (default: 1)"
    )
    gen_ota_parser.add_argument(
        ...
        type=float,
        default=[2.25, 1],
        help="current_mirror_params (default: 2.25 1)",
    )
    gen_ota_parser.add_argument(
        "--resistor_params",
        nargs=4,
        type=float,
        default=[0.5, 3, 4, 4],
        help="resistor_params (default: 0.5 3 4 4)",
    )
    gen_ota_parser.add_argument(
        "--global_current_bias_params",
        nargs=3,
        type=float,
        default=[8.3, 1.42, 2],
        help="global_currrent_bias_params (default: 8.3 1.42 2)",
    )
    gen_ota_parser.add_argument(
        "--output_gds", help="Filename for outputing ota (gen_ota mode only)"
    )
    gen_ota_parser.add_argument(
        "--add_pads", action="store_true", help="add pads (gen_ota mode only)"
    )

    # subparser for gen_otas mode
    gen_otas_parser = subparsers.add_parser(
        "gen_otas",
        help="generates the otas returned in the small parameters list but only saves GDS. Always outputs to ./outputrawotas",
    )
    ...
    test.add_argument(
        "--output_dir", type=Path, default="./", help="Directory for output GDS file"
    )
    test.add_argument(
        "--temp", type=int, default=int(25), help="Simulation temperature"
    )
    test.add_argument(
        "--cload",
        type=float,
        default=float(80),
        help="run simulation with load capacitance units=pico Farads",
    )
    test.add_argument(
        "--noparasitics",
        action="store_true",
        help="specify that parasitics should be removed when simulating",
    )
    ...
    safe_single_build_and_sim.add_argument(
        "--class_pickle_file", type=Path, help="see safe_single_build_and_simulation"
    )
    ...
    for prsr in [gen_ota_parser, test, safe_single_build_and_sim]:
        prsr.add_argument(
            "--no_lvt",
            action="store_true",
            help="do not place any low threshold voltage transistors.",
        )
        prsr.add_argument(
            "--PDK_ROOT",
            type=Path,
            default="/usr/bin/miniconda3/share/pdk/",
            help="path to the sky130 PDK library",
        )
    ...
    if args.mode in ["test", "gen_otas", "safe_single_build_and_sim"]:
        ...
        if not (PDK_ROOT.is_dir()):
            raise ValueError(
                "PDK_ROOT " + str(PDK_ROOT) + " is not a valid directory\n"
            )
        PDK_ROOT = str(PDK_ROOT)
    ...
    if args.mode == "gen_ota":
        ...
        results = safe_single_build_and_simulation(
            ota_parameters_serializer(**params),
            temperature_info[0],
            args.output_dir,
            cload=args.cload,
            noparasitics=args.noparasitics,
            hardfail=True,
        )
        print(results)
        ...
        def create_func(argnparray, indx: int):
            global usepdk
            comp = super_class_AB_OTA(
                usepdk, **ota_parameters_de_serializer(argnparray)
            )
