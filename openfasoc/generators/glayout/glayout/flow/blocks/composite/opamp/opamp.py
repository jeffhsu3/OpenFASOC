...
from glayout.flow.pdk.util.comp_utils import evaluate_bbox, movex, align_comp_to_port
...
def opamp_output_stage_netlist(
    pdk: MappedPDK, output_amp_fet_ref: ComponentReference, biasParams: list
) -> Netlist:
    bias_netlist = current_mirror_netlist(
        pdk, biasParams[0], biasParams[1], biasParams[2]
    )

    output_stage_netlist = Netlist(
        circuit_name="OUTPUT_STAGE", nodes=["VDD", "GND", "IBIAS", "VIN", "VOUT"]
    )
...
def __add_output_stage(
    ...
    opamp_top << L_route(
        pdk, n_to_p_output_route, amp_fet_ref.ports["multiplier_0_gate_W"]
    )
    # route drain of amplifier to vdd
    vdd_route_extension = (
        opamp_top.ymax - opamp_top.ports["pin_vdd_e4"].center[1] + metal_sep
    )
    opamp_top << c_route(
        pdk,
        amp_fet_ref.ports["multiplier_0_drain_N"],
        set_port_orientation(opamp_top.ports["pin_vdd_e4"], "N"),
        width1=5,
        width2=5,
        extension=vdd_route_extension,
        e2glayer="met3",
    )
    ...
    opamp_top << c_route(
        pdk,
        cmirror_ibias.ports["B_drain_E"],
        amp_fet_ref.ports["multiplier_0_source_E"],
        extension=metal_sep,
    )
    # route cmirror: A gate, B gate and A drain together. Then A source and B source to ground
    gate_short = opamp_top << c_route(
        pdk,
        cmirror_ibias.ports["A_gate_E"],
        cmirror_ibias.ports["B_gate_E"],
    ...
    opamp_top << L_route(
        ...
        cmirror_ibias.ports["A_drain_E"],
        viaoffset=False,
        fullbottom=False,
    )
    srcshort = opamp_top << c_route(
        pdk,
        cmirror_ibias.ports["A_source_W"],
        cmirror_ibias.ports["B_source_W"],
        extension=metal_sep,
    )
    opamp_top << straight_route(
        pdk,
        srcshort.ports["con_N"],
        cmirror_ibias.ports["welltie_N_top_met_S"],
        via2_alignment_layer="met2",
    )
    # Route all tap rings together and ground them
    opamp_top << straight_route(
        pdk,
        cmirror_ibias.ports["welltie_N_top_met_N"],
        amp_fet_ref.ports["tie_S_top_met_S"],
    )
    opamp_top << L_route(
        pdk,
        cmirror_ibias.ports["welltie_S_top_met_S"],
        opamp_top.ports["pin_gnd_E"],
        hwidth=4,
    )
    # add ports, add bias/output pin, and return
    psuedo_out_port = movex(
        amp_fet_ref.ports["multiplier_0_source_E"].copy(), 6 * metal_sep
    )
    output_pin = opamp_top << straight_route(
        pdk, amp_fet_ref.ports["multiplier_0_source_E"], psuedo_out_port
    )
    ...
    bias_pin = opamp_top << rectangle(
        size=(5, 3), layer=pdk.get_glayer("met3"), centered=True
    )
    bias_pin.movex(cmirror_ibias.center[0]).movey(
        cmirror_ibias.ports["B_gate_S"].center[1] - bias_pin.ymax - 5 * metal_sep
    )
    opamp_top << straight_route(
        pdk, bias_pin.ports["e2"], cmirror_ibias.ports["B_gate_S"], width=1
    )
...
def opamp_netlist(two_stage_netlist: Netlist, output_stage_netlist: Netlist) -> Netlist:
    ...
    top_level_netlist.connect_netlist(
        ...
        [
            ("VDD", "vdd"),
            ("GND", "gnd"),
            ("DIFFPAIR_BIAS", "diffpairibias"),
            ("VP", "plus"),
            ("VN", "minus"),
            ("CS_BIAS", "commonsourceibias"),
            ("VOUT", "CSoutput"),
        ],
    )
...
def opamp(
    ...
    ****NOTE: change the multiplier option to change the relative sizing of the current mirror ref/output
    ...
    if add_output_stage:
        opamp_top, output_stage_netlist = __add_output_stage(
            pdk,
            opamp_top,
            output_stage_params,
            output_stage_bias,
        ...
        opamp_top.info["netlist"] = opamp_netlist(
            opamp_top.info["netlist"], output_stage_netlist
        )
