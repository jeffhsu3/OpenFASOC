...
from glayout.flow.pdk.util.comp_utils import evaluate_bbox, movex
...
from glayout.flow.blocks.composite.differential_to_single_ended_converter import (
    differential_to_single_ended_converter,
)
from glayout.flow.blocks.composite.opamp.row_csamplifier_diff_to_single_ended_converter import (
    row_csamplifier_diff_to_single_ended_converter,
)
from glayout.flow.blocks.composite.opamp.diff_pair_stackedcmirror import (
...
def __create_and_route_pins(
    ...
    halfmultp_Lsrcport = opamp_top.ports[
        "commonsource_Pamp_L_multiplier_0_source_con_N"
    ]
    halfmultp_Rsrcport = opamp_top.ports[
        "commonsource_Pamp_R_multiplier_0_source_con_N"
    ]
    opamp_top << c_route(
        pdk,
        halfmultp_Lsrcport,
        halfmultp_Rsrcport,
        extension=opamp_top.ymax - halfmultp_Lsrcport.center[1],
        fullbottom=True,
        viaoffset=(False, False),
    )
    # place vdd pin
    vddpin = opamp_top << rectangle(
        size=(5, 3), layer=pdk.get_glayer("met4"), centered=True
    )
    vddpin.movey(opamp_top.ymax)
    # route vdd to source of 2L/2R
    opamp_top << straight_route(
        pdk, opamp_top.ports["pcomps_2L2Rsrcvia_top_met_N"], vddpin.ports["e4"]
    )
    # drain route above vdd pin
    halfmultp_Ldrainport = opamp_top.ports[
        "commonsource_Pamp_L_multiplier_0_drain_con_N"
    ]
    halfmultp_Rdrainport = opamp_top.ports[
        "commonsource_Pamp_R_multiplier_0_drain_con_N"
    ]
    halfmultp_drain_routeref = opamp_top << c_route(
        pdk,
        halfmultp_Ldrainport,
        halfmultp_Rdrainport,
        extension=opamp_top.ymax
        - halfmultp_Ldrainport.center[1]
    ...
    ptop_halfmultp_gate_route = opamp_top << c_route(
        pdk,
        halfmultp_Lgateport,
        halfmultp_Rgateport,
        extension=abs(pmos_comps_ref.ymin - halfmultp_Lgateport.center[1])
        + pdk.get_grule("met5")["min_separation"],
        fullbottom=True,
        viaoffset=(False, False),
    )
    ...
    extensionL = min(
        halfmultn_drain_routeref.ports["con_W"].center[0],
        halfmultp_drain_routeref.ports["con_W"].center[0],
    )
    extensionR = max(
        halfmultn_drain_routeref.ports["con_E"].center[0],
        halfmultp_drain_routeref.ports["con_E"].center[0],
    )
    opamp_top << c_route(
        pdk,
        halfmultn_drain_routeref.ports["con_W"],
        halfmultp_drain_routeref.ports["con_W"],
        extension=abs(opamp_top.xmin - extensionL) + 2,
        cwidth=2,
    )
    n_to_p_output_route = opamp_top << c_route(
        pdk,
        halfmultn_drain_routeref.ports["con_E"],
        halfmultp_drain_routeref.ports["con_E"],
        extension=abs(opamp_top.xmax - extensionR) + 2,
        cwidth=2,
    )
    # top nwell taps to vdd, top p substrate taps to gnd
    opamp_top << straight_route(
        pdk,
        opamp_top.ports["commonsource_cmirror_output_L_tie_N_top_met_N"],
        opamp_top.ports["pcomps_top_ptap_S_top_met_S"],
        width=5,
    )
    opamp_top << straight_route(
        pdk,
        opamp_top.ports["commonsource_cmirror_output_R_tie_N_top_met_N"],
        opamp_top.ports["pcomps_top_ptap_S_top_met_S"],
        width=5,
    )
    ...
    opamp_top << straight_route(
        pdk,
        movex(vddpin.ports["e4"], destination=L_toptapn_route.center[0]),
        L_toptapn_route,
        glayer1="met3",
        fullbottom=True,
    )
    opamp_top << straight_route(
        pdk,
        movex(vddpin.ports["e4"], destination=R_toptapn_route.center[0]),
        R_toptapn_route,
        glayer1="met3",
        fullbottom=True,
    )
    # bias pins for first two stages
    vbias1 = opamp_top << rectangle(
        size=(5, 3), layer=pdk.get_glayer("met3"), centered=True
    )
    vbias1.movey(opamp_top.ymin - _max_metal_seperation_ps - vbias1.ymax)
    opamp_top << straight_route(
        pdk,
        vbias1.ports["e2"],
        opamp_top.ports["diffpair_ibias_B_gate_S"],
        width=1,
        fullbottom=False,
    )
    ...
    vbias2 = opamp_top << rectangle(
        size=(5, 3), layer=pdk.get_glayer("met4"), centered=True
    )
    vbias2.movex(
        1 + opamp_top.xmax + evaluate_bbox(vbias2)[0] + pdk.util_max_metal_seperation()
    ).movey(opamp_top.ymin + vbias2.ymax)
    opamp_top << L_route(
        pdk, halfmultn_gate_routeref.ports["con_E"], vbias2.ports["e2"], hwidth=2
    )
    # route + and - pins (being careful about antenna violations)
    minusi_pin = opamp_top << rectangle(
        size=(5, 2), layer=pdk.get_glayer("met3"), centered=True
    )
    minusi_pin.movex(opamp_top.xmin).movey(
        _max_metal_seperation_ps
        + minusi_pin.ymax
        + halfmultn_drain_routeref.ports["con_W"].center[1]
        + halfmultn_drain_routeref.ports["con_W"].width / 2
    )
    iport_antenna1 = movex(
        minusi_pin.ports["e3"],
        destination=opamp_top.ports["diffpair_MINUSgateroute_W_con_N"].center[0]
        - 9 * _max_metal_seperation_ps,
    )
    opamp_top << L_route(
        pdk, opamp_top.ports["diffpair_MINUSgateroute_W_con_N"], iport_antenna1
    )
    ...
    plusi_pin = opamp_top << rectangle(
        size=(5, 2), layer=pdk.get_glayer("met3"), centered=True
    )
    # PLUS pin lifted 3.5um above the minus pin: the two input rails used to
    # run to the west edge stacked max_metal_seperation (~0.3um) apart for
    # ~34um, putting ~4.5 fF of VOUT<->VIN coupling on each unity-follower
    # (the delay-stage PEX C7/C18) -- enough to limit-cycle the S/H chain at
    # the ff corner (chipforge PEX_RC_SIMS.md sec 6). 3.5um makes the
    # rail-rail term negligible while clearing the vddpin met3 verticals and
    # the halfmultn drain-route met4 above by >0.7um (4.0 grazed them at
    # 0.27um).
    plusi_pin.movex(opamp_top.xmin + plusi_pin.xmax).movey(
        _max_metal_seperation_ps + 3.5 + minusi_pin.ymax + plusi_pin.ymax
    )
    iport_antenna1 = movex(
        plusi_pin.ports["e3"],
        destination=opamp_top.ports["diffpair_PLUSgateroute_E_con_N"].center[0]
        - 9 * _max_metal_seperation_ps,
    )
    opamp_top << L_route(
        pdk, opamp_top.ports["diffpair_PLUSgateroute_E_con_N"], iport_antenna1
    )
    iport_antenna2 = movex(iport_antenna1, offsetx=-9 * _max_metal_seperation_ps)
    opamp_top << straight_route(
        pdk,
    ...
    opamp_top << straight_route(
        pdk,
        opamp_top.ports["diffpair_tr_multiplier_0_drain_N"],
        opamp_top.ports["pcomps_pbottomAB_R_gate_S"],
        glayer1="met5",
        width=3 * pdk.get_grule("met5")["min_width"],
        via1_alignment_layer="met2",
        via1_alignment=("c", "c"),
    )
    opamp_top << straight_route(
        pdk,
        opamp_top.ports["diffpair_tl_multiplier_0_drain_N"],
        opamp_top.ports["pcomps_minusvia_top_met_S"],
        glayer1="met5",
        width=3 * pdk.get_grule("met5")["min_width"],
        via1_alignment_layer="met2",
        via1_alignment=("c", "c"),
    ...
    outputvia_diff_pcomps.movex(
        opamp_top.ports["diffpair_tl_multiplier_0_drain_N"].center[0]
    ).movey(ptop_halfmultp_gate_route.ports["con_E"].center[1])
    # add pin ports
...
def __add_mimcap_arr(
    ...
    mimcaps_ref = opamp_top << mimcap_array(
        pdk, mim_cap_rows, 2, size=mim_cap_size, rmult=6
    )
    ...
    mimcaps_ref.movex(
        pdk.snap_to_2xgrid(opamp_top.xmax + displace_fact + mim_cap_size[0] / 2)
    )
    ...
    cref2_extension = (
        max_metalsep + opamp_top.ymax - max(port1.center[1], port2.center[1])
    )
    ...
    opamp_top << L_route(
        pdk, mimcaps_ref.ports["row0_col0_top_met_S"], intermediate_output, hwidth=3
    )
...
def opamp_gain_stage_netlist(
    mimcap_netlist: Netlist, diff_cs_netlist: Netlist, cs_bias_netlist: Netlist
) -> Netlist:
    netlist = Netlist(
        circuit_name="GAIN_STAGE", nodes=["VIN1", "VIN2", "VOUT", "VDD", "IBIAS", "GND"]
    )

    diff_cs_ref = netlist.connect_netlist(diff_cs_netlist, [("VSS", "VDD")])
    ...
    mimcap_ref = netlist.connect_netlist(
        mimcap_netlist, [("V1", "VOUT"), ("V2", "VSS2")]
    )

    netlist.connect_subnets(mimcap_ref, diff_cs_ref, [("V2", "VSS2")])

    return netlist


def opamp_twostage_netlist(
    input_stage_netlist: Netlist, gain_stage_netlist: Netlist
) -> Netlist:
    ...
    input_stage_ref = two_stage_netlist.connect_netlist(
        input_stage_netlist, [("IBIAS", "DIFFPAIR_BIAS"), ("VSS", "GND"), ("B", "GND")]
    )

    gain_stage_ref = two_stage_netlist.connect_netlist(
        gain_stage_netlist, [("IBIAS", "CS_BIAS")]
    )

    two_stage_netlist.connect_subnets(
        input_stage_ref, gain_stage_ref, [("VDD1", "VIN1"), ("VDD2", "VIN2")]
    )
...
def opamp_twostage(
    ...
    ****NOTE: change the multiplier option to change the relative sizing of the current mirror ref/output
    ...
    if with_antenna_diode_on_diffinputs != 0 and with_antenna_diode_on_diffinputs < 2:
        raise ValueError(
            "number of antenna diodes should be at least 2 (or 0 to specify no diodes)"
        )
    if half_common_source_bias[3] < 2:
        raise ValueError("half_common_source_bias num multiplier must be >= 2")
    opamp_top, halfmultn_drain_routeref, halfmultn_gate_routeref, _cref = (
        diff_pair_stackedcmirror(
            pdk,
            ...
            diffpair_plus_minus_sep=diffpair_plus_minus_sep,
        )
    )
    ...
    pmos_comps = differential_to_single_ended_converter(
        pdk,
        rmult,
        half_pload,
        opamp_top.ports["diffpair_tl_multiplier_0_drain_N"].center[0],
    ...
    pmos_comps = row_csamplifier_diff_to_single_ended_converter(
        pdk,
        pmos_comps,
    ...
    cs_bias_netlist = current_mirror_netlist(
        pdk,
        width=half_common_source_bias[0],
        length=half_common_source_bias[1],
        multipliers=half_common_source_bias[2],
    )
    ...
    rename_func = (
        lambda name_, port_: name_.replace(
            "pcomps_halfpspecialmarker", "commonsource_Pamp"
        )
        if name_.startswith("pcomps_halfpspecialmarker")
        else name_
    )
    opamp_top = rename_component_ports(opamp_top, rename_function=rename_func)
    # create pins and route
    clear_cache()
    opamp_top, n_to_p_output_route = __create_and_route_pins(
        pdk,
        opamp_top,
        pmos_comps_ref,
        halfmultn_drain_routeref,
        halfmultn_gate_routeref,
    )
    # place mimcaps and route
    clear_cache()
    opamp_top, mimcap_netlist = __add_mimcap_arr(
        pdk,
        opamp_top,
        mim_cap_size,
        mim_cap_rows,
    ...
    pmos_comps.info["netlist"] = opamp_gain_stage_netlist(
        mimcap_netlist, pmos_comps.info["netlist"], cs_bias_netlist
    )
    opamp_top.info["netlist"] = opamp_twostage_netlist(
        opamp_top.info["netlist"], pmos_comps.info["netlist"]
    )
