...
# Port -> LVS-label mapping; layers derived from each port's own metal (PDK-agnostic).
_DF_LABELS = [
    LabelSpec("VTAIL", "bl_multiplier_0_source_S", size=0.27),
    LabelSpec("VDD1", "tl_multiplier_0_drain_N", size=0.27),
    LabelSpec("VDD2", "tr_multiplier_0_drain_N", size=0.27),
    LabelSpec("B", "tap_N_top_met_S", size=0.5),
    LabelSpec("VP", "br_multiplier_0_gate_S", size=0.27),
    LabelSpec("VN", "bl_multiplier_0_gate_S", size=0.27),
]


def add_df_labels(df_in: Component, pdk: MappedPDK) -> Component:
    """Add LVS pin rectangles + text labels to a differential pair (PDK-agnostic)."""
    return add_pin_labels(df_in, pdk, _DF_LABELS)


def diff_pair_netlist(fetL: Component, fetR: Component) -> Netlist:
    diff_pair_netlist = Netlist(
        circuit_name="DIFF_PAIR", nodes=["VP", "VN", "VDD1", "VDD2", "VTAIL", "B"]
    )
    diff_pair_netlist.connect_netlist(
        fetL.info["netlist"], [("D", "VDD1"), ("G", "VP"), ("S", "VTAIL"), ("B", "B")]
    )
    diff_pair_netlist.connect_netlist(
        fetR.info["netlist"], [("D", "VDD2"), ("G", "VN"), ("S", "VTAIL"), ("B", "B")]
    )
...
def diff_pair(
    ...
    if n_or_p_fet:
        ...
        min_spacing_x = pdk.get_grule("p+s/d")["min_separation"] - 2 * (
            fetL.xmax - fetL.ports["multiplier_0_plusdoped_E"].center[0]
        )
        well = "nwell"
    # place transistors
    ...
    metal_min_dim = max(
        pdk.get_grule("met2")["min_width"], pdk.get_grule("met3")["min_width"]
    )
    metal_space = max(
    ...
    gate_route_os = (
        evaluate_bbox(viam2m3)[0]
        - fetL.ports["multiplier_0_gate_W"].width
        + metal_space
    )
    ...
    a_topl = (
        (diffpair << fetL)
        .movey(fetL.ymax + min_spacing_y / 2 + 0.5)
        .movex(0 - fetL.xmax - min_spacing_x / 2)
    )
    b_topr = (
        (diffpair << fetR)
        .movey(fetR.ymax + min_spacing_y / 2 + 0.5)
        .movex(fetL.xmax + min_spacing_x / 2)
    )
    a_botr = diffpair << fetR
    a_botr.mirror_y().movey(0 - 0.5 - fetL.ymax - min_spacing_y / 2).movex(
        fetL.xmax + min_spacing_x / 2
    )
    b_botl = diffpair << fetL
    b_botl.mirror_y().movey(0 - 0.5 - fetR.ymax - min_spacing_y / 2).movex(
        0 - fetL.xmax - min_spacing_x / 2
    )
    # if substrate tap place substrate tap
    if substrate_tap:
        tapref = diffpair << tapring(
            pdk, evaluate_bbox(diffpair, padding=1), horizontal_glayer="met1"
        )
        diffpair.add_ports(tapref.get_ports_list(), prefix="tap_")
        try:
            diffpair << straight_route(
                pdk,
                a_topl.ports["multiplier_0_dummy_L_gsdcon_top_met_W"],
                diffpair.ports["tap_W_top_met_W"],
                glayer2="met1",
            )
        except KeyError:
            pass
        try:
            diffpair << straight_route(
                pdk,
                b_topr.ports["multiplier_0_dummy_R_gsdcon_top_met_W"],
                diffpair.ports["tap_E_top_met_E"],
                glayer2="met1",
            )
        except KeyError:
            pass
        try:
            diffpair << straight_route(
                pdk,
                b_botl.ports["multiplier_0_dummy_L_gsdcon_top_met_W"],
                diffpair.ports["tap_W_top_met_W"],
                glayer2="met1",
            )
        except KeyError:
        ...
        try:
            diffpair << straight_route(
                pdk,
                a_botr.ports["multiplier_0_dummy_R_gsdcon_top_met_W"],
                diffpair.ports["tap_E_top_met_E"],
                glayer2="met1",
            )
        except KeyError:
    ...
    dextension = (
        source_routeE.xmax
        - b_topr.ports["multiplier_0_drain_E"].center[0]
        + metal_space
    )
    ...
    def get_left_extension(bar):
        return (
            abs(
                diffpair.xmin
                - min(
                    a_topl.ports["multiplier_0_gate_W"].center[0],
                    bar.ports["e1"].center[0],
                )
            )
            + pdk.get_grule("met2")["min_separation"]
        )

    def get_right_extension(bar):
        return (
            abs(
                diffpair.xmax
                - max(
                    b_topr.ports["multiplier_0_gate_E"].center[0],
                    bar.ports["e3"].center[0],
                )
            )
            + pdk.get_grule("met2")["min_separation"]
        )
    ...
    bar_comp = rectangle(
        centered=True,
        size=(
            abs(b_topr.xmax - a_topl.xmin),
            b_topr.ports["multiplier_0_gate_E"].width,
        ),
        layer=pdk.get_glayer("met2"),
    )
    ...
    plus_minus_seperation = max(
        pdk.get_grule("met2")["min_separation"], plus_minus_seperation
    )
    bar_minus = (diffpair << bar_comp).movey(
        diffpair.ymax + bar_comp.ymax + plus_minus_seperation
    )
...
def diff_pair_generic(
    ...
    diffpair = common_centroid_ab_ba(
        pdk, width, fingers, length, n_or_p_fet, rmult, dummy, substrate_tap
    )
    diffpair << smart_route(
        pdk,
        diffpair.ports["A_source_E"],
        diffpair.ports["B_source_E"],
        diffpair,
        diffpair,
    )
    return diffpair
