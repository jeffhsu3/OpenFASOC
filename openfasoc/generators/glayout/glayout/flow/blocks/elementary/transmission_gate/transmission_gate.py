...
def tg_netlist(nfet: Component, pfet: Component) -> Netlist:
    ...
    netlist.connect_netlist(
        nfet.info["netlist"], [("D", "VOUT"), ("G", "VGN"), ("S", "VIN"), ("B", "VSS")]
    )
    netlist.connect_netlist(
        pfet.info["netlist"], [("D", "VOUT"), ("G", "VGP"), ("S", "VIN"), ("B", "VCC")]
    )
    ...
    **kwargs are any kwarg that is supported by nmos and pmos
    ...
    if tie_layers[0] != "met2":
        ...
        def _cbar(p1, p2, east, name):
            ...
            ext, bw = 0.5, 0.4
            x_edge = (
                max(sgn * p1.center[0], sgn * p2.center[0]) * sgn
            )  # outermost rail end
            ...
            # Port at the trunk centre -- the default place to connect this net (VIN=source,
            # VOUT=drain): a met2 landing between the two FETs, off the per-FET rails.
            top_level.add_port(
                name=name,
        ...
        # trailing "_x" is a placeholder segment: rename_ports_by_orientation replaces the
        # LAST underscore-segment with the orientation letter -> "source_con_E"/"drain_con_W".
        _cbar(
            nfet_ref.ports[f"multiplier_{_mn}_source_E"],
            pfet_ref.ports[f"multiplier_{_mp}_source_E"],
            east=True,
            name="source_con_x",
        )
        _cbar(
            nfet_ref.ports[f"multiplier_{_mn}_drain_W"],
            pfet_ref.ports[f"multiplier_{_mp}_drain_W"],
            east=False,
            name="drain_con_x",
        )
    else:
        ...
        top_level << c_route(
            pdk,
            nfet_ref.ports["multiplier_0_source_E"],
            pfet_ref.ports["multiplier_0_source_E"],
            extension=src_extension,
        )
        top_level << c_route(
            pdk,
            nfet_ref.ports["multiplier_0_drain_W"],
            pfet_ref.ports["multiplier_0_drain_W"],
            viaoffset=False,
        )
    ...
    if substrate_tap:
        substrate_tap_encloses = (
            (evaluate_bbox(top_level)[0] + pdk.util_max_metal_seperation() + 0.02),
            (evaluate_bbox(top_level)[1] + pdk.util_max_metal_seperation() + 0.02),
        )
        ...
        guardring_ref.move(nfet_ref.center).movey(
            evaluate_bbox(pfet_ref)[1] / 2
            + (pdk.util_max_metal_seperation() + 0.02) / 2
        )
...
if __name__ == "__main__":
    ...
    for pdk_name, pdk in [("gf180", gf180_mapped_pdk), ("sky130", sky130_mapped_pdk)]:
        ...
        comp = transmission_gate(
            pdk, multipliers=(2, 2), fingers=(2, 1), tie_layers=("met1", "met1")
        )
